...
import { 
  SendHorizontal, Bot, User as UserIcon, Loader2, 
  Sparkles, X, MessageCircle, BookOpen, ArrowUpRight 
} from "lucide-react";
...
const ChatHeader = ({ onClose }: { onClose: () => void }) => (
  <header className="p-4 border-b border-zinc-100 bg-zinc-900 text-white flex items-center justify-between">
    <div className="flex items-center gap-3">
      ...
      <div>
        <h3 className="text-[11px] font-bold uppercase tracking-widest leading-none">Galaxy Assistant</h3>
        <div className="flex items-center gap-1.5 mt-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
          <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-tighter">Systems Live</span>
        </div>
      </div>
    </div>
    <button onClick={onClose} className="p-1.5 hover:bg-zinc-800 rounded-md transition-colors text-zinc-400 hover:text-white">
      <X size={18} />
    </button>
  </header>
);
  ...
  const handleSend = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        setChat(prev => [...prev, { role: 'assistant', text: data.reply || data.error }]);
      } catch (err) {
        setChat(prev => [...prev, { role: 'assistant', text: "Service temporarily unavailable." }]);
      }
    });
  };
  ...
  return (
    <div className="fixed lg:bottom-6 bottom-18 right-6 z-[100] flex flex-col items-end">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-6 scroll-smooth">
          {chat.length === 0 && (
            <div className="py-12 text-center space-y-4">
              ...
              <div>
                <h4 className="text-sm font-boldtracking-tight">AI Bookstore Assistant</h4>
                <p className="text-xs text-zinc-500 mt-1 px-12">Search the catalog, check inventory, or get reading recommendations.</p>
              </div>
            </div>
          )}
          ...
          {chat.map((msg, i) => (
            <div key={i} className={cn("flex gap-3", msg.role === 'user' ? "flex-row-reverse" : "flex-row")}>
              <div className={cn(
                ...
                msg.role === 'user' ? "bg-background border-zinc-900 " : "bg-background border-zinc-200"
              )}>
                {msg.role === 'user' ? <UserIcon /> : <Bot size={14} />}
              </div>
              <div className={cn(
                "max-w-[80%] p-3.5 rounded-2xl text-sm leading-relaxed shadow-sm transition-all",
                msg.role === 'user' ? "bg-zinc-900 text-white" : "bg-zinc-900 "
              )}>
                {msg.text}
              </div>
            </div>
      ...
      >
        ...
        {!isOpen && <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-tr from-white/10 to-transparent pointer-events-none" />}
        {!isOpen && <div className="absolute top-3 right-3 w-2.5 h-2.5 bg-emerald-500 rounded-full border-2 border-zinc-900" />}
      </button>
    </div>
  );
}